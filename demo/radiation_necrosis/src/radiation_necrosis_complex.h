...
// Forward declare behavior classes
...
// Behavior implementations - after namespace to avoid ROOT issues

// RadiationTherapy constructor
bdm::radiation_necrosis::RadiationTherapy::RadiationTherapy(double dose, int treatment_step) 
    : dose_(dose), treatment_step_(treatment_step), applied_(false) {}

// RadiationTherapy Run method
void bdm::radiation_necrosis::RadiationTherapy::Run(bdm::Agent* agent) {
  using namespace bdm::radiation_necrosis;
...
// CellDeath constructor
bdm::radiation_necrosis::CellDeath::CellDeath(int treatment_step) : treatment_step_(treatment_step) {}

// CellDeath Run method
void bdm::radiation_necrosis::CellDeath::Run(bdm::Agent* agent) {
  using namespace bdm::radiation_necrosis;
...
// InflammatoryResponse Run method
void bdm::radiation_necrosis::InflammatoryResponse::Run(bdm::Agent* agent) {
  using namespace bdm::radiation_necrosis;
  ...
  // Simplified inflammation response - just add some random inflammation over time
  // and inflammation-induced damage
  if (random->Uniform() < 0.1) { // 10% chance per step
    double inflammation_increase = 0.05 * random->Uniform(0.8, 1.2);
    cell->SetInflammationLevel(cell->GetInflammationLevel() + inflammation_increase);
    
    // Inflammation causes additional damage over time
    double inflammation_damage = cell->GetInflammationLevel() * 0.05;
    cell->SetDamageLevel(cell->GetDamageLevel() + inflammation_damage);
    cell->SetMetabolicHealth(cell->GetMetabolicHealth() - inflammation_damage * 0.05);
  }
  
  // Natural inflammation decay
  if (cell->GetInflammationLevel() > 0.1) {
    cell->SetInflammationLevel(cell->GetInflammationLevel() * 0.98);
  }
}
